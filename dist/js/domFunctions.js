...
export const updateDisplay = (weatherJson, locationObj) => {
  ...
  displaysevenDayForecast(weatherJson);
  setFocusOnSearch();
  fadeDisplay();
};
...
const fadeDisplay = () => {
  ...
  const sevenDay = document.getElementById("dailyForecast");
  sevenDay.classList.toggle("zero-vis");
  sevenDay.classList.toggle("fade-in");
};
...
const clearDisplay = () => {
  ...
  const sevenDayForecast = document.getElementById("dailyForecast__contents");
  deleteContents(sevenDayForecast);
};
...
const buildScreenReaderWeather = (weatherJson, locationObj) => {
  ...
  const tempUnit = unit === "Metric" ? "Fahrenheit" : "Celsius";
...
const displaysevenDayForecast = (weatherJson) => {
  for (let i = 1; i <= 7; i++) {
...
const createDailyForecastDivs = (dayWeather) => {
  ...
  const dateValueeString = getDateAbbreviation(dayWeather.dt);
  const DateAbbreviation = createElem(
    "p",
    "dateValuee",
   dateValueeString
  );
  ...
  return [ dayAbbreviation,DateAbbreviation, dayIcon, dayHigh, dayLow];
};
...
const getDateAbbreviation = (data) => {
  const dateObj = new Date(data * 1000);
  const utcString = dateObj.toISOString();
  return utcString.slice(5, 10);
};
